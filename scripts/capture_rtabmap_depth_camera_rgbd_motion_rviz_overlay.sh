...
RESULT_DIR="${RESULT_ROOT}/rtabmap_depth_camera_rgbd_motion_rviz_overlay_${STAMP}"
SUMMARY_FILE="${RESULT_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_overlay_${STAMP}.txt"
AGENT_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_agent_${STAMP}.log"
PX4_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_px4_${STAMP}.log"
PX4_WRAPPER_LOG="${PX4_LOG}.wrapper"
OFFBOARD_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_offboard_${STAMP}.log"
BRIDGE_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_odom_bridge_${STAMP}.log"
RTABMAP_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_rtabmap_${STAMP}.log"
RVIZ_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_rviz_${STAMP}.log"
STATUS_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_vehicle_status_${STAMP}.log"
LOCAL_POSITION_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_vehicle_local_position_${STAMP}.log"
LOCAL_POSITION_TRAJECTORY_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_vehicle_local_position_trajectory_${STAMP}.log"
DEPTH_POSE_TRAJECTORY_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_depth_pose_trajectory_${STAMP}.log"
TOPICS_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_topics_${STAMP}.log"
RGB_SAMPLE_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_rgb_sample_${STAMP}.log"
DEPTH_SAMPLE_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_depth_sample_${STAMP}.log"
SCREENSHOT_FILE="${SCREENSHOT_DIR}/rtabmap_depth_camera_rgbd_motion_rviz_overlay_${STAMP}.png"
...
if [[ -z "${DISPLAY:-}" ]]; then
  echo "DISPLAY is not set; cannot capture RTAB-Map RGB-D motion RViz screenshot." >&2
  exit 1
fi
if [[ ! -x "${AGENT_BIN}" ]]; then
...
if [[ "${rtabmap_ok}" != "true" || "${motion_ok}" != "true" || "${screenshot_ok}" != "1" || ! -s "${SCREENSHOT_FILE}" ]]; then
  decision="rejected_rtabmap_depth_camera_rgbd_motion_rviz_overlay"
  reason="rgbd_motion_rviz_capture_missing_motion_or_screenshot_evidence"
fi

{
  echo "scope=rtabmap_depth_camera_rgbd_motion_rviz_overlay"
...
echo "RTAB-Map RGB-D motion RViz overlay capture completed."
echo "Summary: ${SUMMARY_FILE}"

if [[ "${decision}" != "accepted_rtabmap_depth_camera_rgbd_motion_rviz_overlay" ]]; then
  exit 1
fi
