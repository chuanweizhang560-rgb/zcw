...
for file in "${TRACKING_SUMMARY}" "${FRAME_SUMMARY}" "${COVERAGE_SUMMARY}" "${COVERAGE_CSV}"; do
  if [[ ! -f "${file}" ]]; then
...
python3 - "${TRACKING_SUMMARY}" "${FRAME_SUMMARY}" "${COVERAGE_SUMMARY}" "${COVERAGE_CSV}" \
...
boundary_ok = all(
    ...
    for kv in (tracking, frame, coverage)
)

accepted = tracking_ok and frame_ok and coverage_ok and boundary_ok
