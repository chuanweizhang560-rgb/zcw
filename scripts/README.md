...
- `verify_four_vehicle_dry_run_smoke.sh`：启动 PX4/Gazebo Classic 四机 read-only、Micro XRCE-DDS 和 `four_vehicle_dry_run_planner`，验证 `/zcw/multi_vehicle/four_vehicle_dry_run/*` 话题发布，并确认 `/px4_1` 到 `/px4_4` 关键 `/fmu/in/*` topic publisher count 仍为 `0`；默认不启动 RViz，可用 `CAPTURE_RVIZ=1` 加载 `four_vehicle_dry_run_overlay.rviz` 截图；不启动 Offboard、不 arm。
- `audit_four_vehicle_dry_run_samples.sh`：只读解析 `verify_four_vehicle_dry_run_smoke.sh` 生成的 dry-run sample 日志，检查四个 goal、topology/safety/assignment、四机规则角色字段、非学习策略标记和 no-active/no-fmu-in 标记；不启动 ROS/PX4/Gazebo/RViz。
