...
- `verify_four_vehicle_dry_run_smoke.sh`：启动 PX4/Gazebo Classic 四机 read-only、Micro XRCE-DDS 和 `four_vehicle_dry_run_planner`，验证 `/zcw/multi_vehicle/four_vehicle_dry_run/*` 话题发布，并确认 `/px4_1` 到 `/px4_4` 关键 `/fmu/in/*` topic publisher count 仍为 `0`；不启动 Offboard、不 arm。
