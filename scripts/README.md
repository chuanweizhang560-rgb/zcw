...
- `audit_two_vehicle_dry_run_contract.sh`：只读静态审计 two-vehicle dry-run planner 的源码、launch 和 CMake，确认只订阅 `/px4_1/fmu/out/*`、`/px4_2/fmu/out/*`，只发布 `/zcw/multi_vehicle/dry_run/*`，没有 PX4 input topic、Offboard/arm 或 cable active 连接；不启动 ROS/PX4/Gazebo/RViz。
- `verify_two_vehicle_dry_run_smoke.sh`：启动 PX4/Gazebo Classic 双机 read-only、Micro XRCE-DDS 和 `two_vehicle_dry_run_planner`，验证 `/zcw/multi_vehicle/dry_run/*` 话题发布，并确认 `/px4_1/fmu/in/*`、`/px4_2/fmu/in/*` 关键 topic publisher count 仍为 `0`；默认不启动 RViz，可用 `CAPTURE_RVIZ=1` 加载 `two_vehicle_dry_run_overlay.rviz` 截图；不启动 Offboard、不 arm。
