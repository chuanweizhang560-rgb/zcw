...
- `verify_two_vehicle_dry_run_smoke.sh`：启动 PX4/Gazebo Classic 双机 read-only、Micro XRCE-DDS 和 `two_vehicle_dry_run_planner`，验证 `/zcw/multi_vehicle/dry_run/*` 话题发布，并确认 `/px4_1/fmu/in/*`、`/px4_2/fmu/in/*` 关键 topic publisher count 仍为 `0`；不启动 Offboard、不 arm。
