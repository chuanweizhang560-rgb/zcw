...
RESULT_DIR="${RESULT_ROOT}/rtabmap_depth_camera_rgbd_rviz_overlay_${STAMP}"
SUMMARY_FILE="${RESULT_DIR}/rtabmap_depth_camera_rgbd_rviz_overlay_${STAMP}.txt"
PX4_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_rviz_px4_${STAMP}.log"
BRIDGE_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_rviz_odom_bridge_${STAMP}.log"
RTABMAP_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_rviz_node_${STAMP}.log"
RVIZ_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_rviz_${STAMP}.log"
TOPICS_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_rviz_topics_${STAMP}.log"
RGB_SAMPLE_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_rviz_rgb_sample_${STAMP}.log"
DEPTH_SAMPLE_LOG="${LOG_DIR}/rtabmap_depth_camera_rgbd_rviz_depth_sample_${STAMP}.log"
SCREENSHOT_FILE="${SCREENSHOT_DIR}/rtabmap_depth_camera_rgbd_rviz_overlay_${STAMP}.png"
...
setsid env \
  ...
  bash -lc "source /opt/ros/humble/setup.bash && source '${ROOT_DIR}/install/setup.bash' && ros2 run rtabmap_slam rtabmap --ros-args \
    -p database_path:='${RESULT_DIR}/rtabmap_depth_camera_rgbd_rviz_${STAMP}.db' \
...
if [[ "${rtabmap_ok}" != "true" || "${outputs_ok}" != "true" || "${screenshot_ok}" != "1" || ! -s "${SCREENSHOT_FILE}" ]]; then
  decision="rejected_rtabmap_depth_camera_rgbd_rviz_overlay"
  reason="rviz_capture_missing_rgbd_topic_or_screenshot_evidence"
fi

{
  echo "scope=rtabmap_depth_camera_rgbd_rviz_overlay"
...
echo "RTAB-Map depth camera RGB-D RViz overlay capture completed."
echo "Summary: ${SUMMARY_FILE}"

if [[ "${decision}" != "accepted_rtabmap_depth_camera_rgbd_rviz_overlay" ]]; then
