...
DEFAULT_OCCLUSION_SUMMARY="data/results/cable_multiview_surface_occlusion_offline_20260617_162441/cable_multiview_surface_occlusion_offline_20260617_162441.txt"
...
for file in "${C1_SUMMARY}" "${C2_CANDIDATE_SUMMARY}" "${C2_UNION_SUMMARY}" "${OCCLUSION_SUMMARY}" "${DRY_RUN_SUMMARY}"; do
  if [[ ! -f "${file}" ]]; then
...
occlusion_decision="$(read_field "${OCCLUSION_SUMMARY}" decision)"
occlusion_claim="$(read_field "${OCCLUSION_SUMMARY}" claims_cable_multiview_surface_occlusion_offline_pass)"
occlusion_ratio="$(read_field "${OCCLUSION_SUMMARY}" global_min_occlusion_clear_total_surface_ratio)"
occlusion_final_claim="$(read_field "${OCCLUSION_SUMMARY}" claims_final_cable_inspection_coverage)"
...
python3 - "${SUMMARY_FILE}" \
  "${C1_SUMMARY}" "${C2_CANDIDATE_SUMMARY}" "${C2_UNION_SUMMARY}" "${OCCLUSION_SUMMARY}" "${DRY_RUN_SUMMARY}" \
  ...
  "${occlusion_decision}" "${occlusion_claim}" "${occlusion_ratio}" "${occlusion_final_claim}" \
  "${dry_run_decision}" "${dry_run_claim}" "${MIN_MULTIVIEW_TOTAL_RATIO}" "${MIN_OCCLUSION_CLEAR_TOTAL_RATIO}" <<'PY'
import sys
...
occlusion_ratio_f = float(occlusion_ratio)
min_multiview_total_ratio_f = float(min_multiview_total_ratio)
min_occlusion_clear_total_ratio_f = float(min_occlusion_clear_total_ratio)
...
occlusion_ok = (
    occlusion_decision == "accepted_cable_multiview_surface_occlusion_offline"
    and occlusion_claim == "true"
    and occlusion_ratio_f >= min_occlusion_clear_total_ratio_f
)
dry_run_ok = dry_run_decision == "accepted_cable_dry_run_acceptance" and dry_run_claim == "true"
final_claim_blocked = c1_final_claim == "false" and c2_final_claim == "false" and occlusion_final_claim == "false"
accepted = c1_ok and c2_candidate_ok and c2_union_ok and occlusion_ok and dry_run_ok and final_claim_blocked
...
lines = [
    ...
    f"occlusion_global_min_clear_total_surface_ratio={occlusion_ratio_f:.9f}",
