...
RESULT_DIR="${RESULT_ROOT}/rtabmap_wind_trajectory_ate_${STAMP}"
SUMMARY_FILE="${RESULT_DIR}/rtabmap_wind_trajectory_ate_${STAMP}.txt"
CSV_FILE="${RESULT_DIR}/rtabmap_wind_trajectory_ate_${STAMP}.csv"
...
if [[ -z "${SOURCE_SUMMARY}" || ! -f "${SOURCE_SUMMARY}" ]]; then
  echo "wind RTAB-Map summary not found" >&2
  exit 1
fi
...
DB_PATH="${DB_PATH:-$(find "$(dirname "${SOURCE_SUMMARY}")" -maxdepth 1 -type f -name '*.db' | sort | tail -n 1)}"
REFERENCE_LOG="${REFERENCE_LOG:-$(value_for depth_pose_trajectory_log "${SOURCE_SUMMARY}")}"
...
if [[ -z "${REFERENCE_LOG}" || ! -f "${REFERENCE_LOG}" ]]; then
  echo "reference trajectory log not found: ${REFERENCE_LOG}" >&2
  exit 1
fi

mkdir -p "${RESULT_DIR}"

python3 - "$DB_PATH" "$REFERENCE_LOG" "$CSV_FILE" "$SUMMARY_FILE" "$SOURCE_SUMMARY" <<'PY'
import csv
import math
import re
import sqlite3
...
source_summary = Path(sys.argv[5])
...
def load_rtabmap_poses(path):
    ...
    for node_id, stamp, blob in rows:
        ...
        # RTAB-Map stores Transform as a 3x4 row-major matrix.
...
def parse_reference_log(path):
    ...
    # Deduplicate identical stamps by keeping the last sample.
    dedup = []
...
decision = "accepted_rtabmap_wind_trajectory_ate" if computed else "rejected_rtabmap_wind_trajectory_ate"
...
lines = [
    "scope=rtabmap_wind_trajectory_ate",
    f"decision={decision}",
    f"reason={reason}",
    "starts_ros=false",
    "starts_px4=false",
    ...
    "rotation_row_major="
    + " ".join(f"{v:.9f}" for v in rotation.reshape(-1)),
...
echo "RTAB-Map wind trajectory ATE audit completed."
